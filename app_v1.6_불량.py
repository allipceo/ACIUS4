# app_v1.6.py - ACIU S4 Week2 메인 앱 (수정 버전)
...
def create_app():
    ...
    # 통계 라우트 (새 파일) - 중복 방지를 위해 다른 prefix 사용
    try:
        from routes.statistics_routes import statistics_bp
        app.register_blueprint(statistics_bp, url_prefix='/statistics')
        print("✅ 새 통계 라우트 등록 완료")
    except ImportError as e:
        print(f"⚠️ 새 통계 라우트 import 실패: {e}")
    ...
    try:
        ...
        app.register_blueprint(stats_routes, url_prefix='/legacy-stats')  # 중복 방지
        print("✅ 기존 통계 라우트 등록 완료")
    except ImportError:
        print("⚠️ 기존 통계 라우트 없음 (정상)")
    ...
    def basic_learning():
        ...
        if os.path.exists('templates/basic_learning.html'):
            ...
            <h1>기본학습</h1>
            <p>사용자 ID: {current_user_id}</p>
            <p>⚠️ templates/basic_learning.html 파일이 없습니다.</p>
            <a href="/home">홈으로</a>
            """
...
if __name__ == '__main__':
    ...
    print("="*60)
    print("🚀 ACIU Season 4 Week 2 서버 시작 (수정 버전)")
