# app_v3.1.py - AICU S4 Blueprint 충돌 완전 해결 버전
...
def create_app():
    """AICU S4 v3.1 - Blueprint 충돌 완전 해결 버전"""
    app = Flask(__name__)
    ...
    # 🔧 v3.1 수정: Blueprint 등록 완전 제거 (충돌 방지)
    register_blueprints_v3_1(app)
    ...
    # 🔧 v3.1 수정: 현재 사용자 정보 API 완전 개선
    @app.route('/user/api/users/current')
    def get_current_user_fixed():
        """현재 사용자 정보 반환 - JavaScript 호환성 완전 개선"""
        print("=== 현재 사용자 조회 API 호출 (v3.1) ===")
        ...
        print(f"🔍 게스트 모드: {is_guest}")
        
        # 🔧 v3.1 수정: JavaScript에서 기대하는 형식으로 응답
        user_data = {
            'userId': current_user_id,  # JavaScript에서 사용하는 필드명
            'userName': user_name,      # JavaScript에서 사용하는 필드명
            'registrationDate': session.get('registration_date', '2025-08-10'),
            'examSubject': session.get('exam_subject', 'ACIU'),
            'examDate': session.get('exam_date', '2025-09-13'),
            'isGuest': is_guest,
            'guestStartTime': session.get('guest_start_time')
        }
        
        print(f"✅ 사용자 정보 반환: {user_data}")
        
        return jsonify({
            'success': True,
            'userData': user_data,  # JavaScript에서 기대하는 형식
            'message': f'{user_name}님의 정보를 성공적으로 조회했습니다.'
        })
    ...
    # 🔧 v3.1 수정: 통계 API 완전 개선
    @app.route('/user/api/users/<user_id>/statistics', methods=['GET'])
    def get_user_statistics_v3_1(user_id):
        """사용자별 통계 조회 - v3.1 완전 개선"""
        print(f"=== 사용자 통계 조회 (v3.1): {user_id} ===")
        ...
        try:
            ...
            # 🔧 v3.1 수정: 현재 세션 ID와 요청 ID가 다르면 현재 세션 ID로 통계 반환
    ...
    # 🔧 v3.1 수정: Week2 API 완전 덮어쓰기 - 퀴즈 시작
    ...
    # 🔧 v3.1 수정: Week2 API 완전 덮어쓰기 - 문제 로딩
    @app.route('/api/quiz/question/<session_id>/<int:index>', methods=['GET'])
    def get_question_v3_1(session_id, index):
        """특정 문제 조회 - v3.1 완전 개선"""
        print(f"=== 문제 조회 (v3.1): {session_id}, {index} ===")
        ...
        try:
            ...
            # 🔧 v3.1 수정: 세션 ID 불일치 시 현재 세션 ID로 세션 생성
    ...
    # 🔧 v3.1 수정: Week2 API 완전 덮어쓰기 - 답안 제출
    @app.route('/api/quiz/submit', methods=['POST'])
    def submit_answer_v3_1():
        """답안 제출 - v3.1 완전 개선"""
        print("=== 답안 제출 (v3.1) ===")
    ...
    # 🔧 v3.1 추가: 현재 사용자 ID 반환 API (JavaScript용)
...
def register_blueprints_v3_1(app):
    """Blueprint 등록 - v3.1 충돌 방지 버전"""
    
    # 🔧 v3.1 수정: Week2 퀴즈 API Blueprint 등록 완전 제거 (충돌 방지)
    print("⚠️ Week2 퀴즈 API Blueprint 등록 제거 (충돌 방지)")
...
if __name__ == '__main__':
    app = create_app()
    print("=" * 60)
    print("🚀 AICU S4 v3.1 FINAL (Blueprint 충돌 완전 해결)")
    print("📍 URL: http://localhost:5000")
    print("📋 v3.1 개선 사항:")
    print("   ✅ Week2 퀴즈 API Blueprint 등록 완전 제거")
    ...
    print("   ✅ Week2 API 완전 덮어쓰기 (퀴즈 시작/문제 로딩/답안 제출)")
