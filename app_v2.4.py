# app_v2.4.py - AICU S4 게스트 모드 자동 등록 및 라우팅 수정 버전
...
def create_app():
    """AICU S4 v2.4 - 게스트 모드 자동 등록 및 라우팅 수정 버전"""
    app = Flask(__name__)
    ...
    @app.route('/')
    def index():
        """홈페이지 - 세션 없을 경우 홍길동으로 자동 등록 후 기본학습으로 리다이렉트"""
        print("=== 홈페이지 접속 ===")
        if 'current_user_id' not in session:
            # 세션이 없을 경우, 홍길동 게스트 세션을 자동으로 생성
            ...
            print(f"✅ '홍길동' 게스트 세션 자동 생성 완료: {guest_id}")
        
        # 수정: 항상 basic_learning으로 리다이렉트
        return redirect(url_for('learning_routes.basic_learning'))
...
if __name__ == '__main__':
    app = create_app()
    print("=" * 60)
    print("🚀 AICU S4 v2.4 GUEST MODE (게스트 모드 자동 등록 및 라우팅 수정)")
    print("📍 URL: http://localhost:5000")
    print("📋 v2.4 새로운 기능:")
    print("   ✅ before_request 로직 제거")
    print("   ✅ 첫 접속 시 '홍길동' 게스트 세션 자동 생성")
    print("   ✅ 바로 기본학습 페이지로 리다이렉트")
