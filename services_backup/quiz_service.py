...
작성자: 노팀장
작성일: 2025년 8월 9일
수정일: 2025년 8월 9일 20:20 KST (스코프 오류 수정)
...
class QuizService:
    ...
    def __init__(self):
        ...
        self.stats_available = False  # 인스턴스 속성으로 관리
        ...
        try:
            ...
            print(f"⚠️ QuizService: StatsHandler 모듈 없음 - {e}")
            self.stats_available = False
        except Exception as e:
            print(f"⚠️ QuizService: StatsHandler 초기화 실패 - {e}")
            self.stats_available = False
    ...
    def get_status(self):
        ...
        return {
            ...
            'stats_handler_available': self.stats_available and self.stats_handler is not None,
            'initialized': True
        }
    ...
    def submit_answer(self, question_index, user_answer):
        ...
        try:
            ...
            # StatsHandler에 기록 (시그니처 어댑터 방식)
            if self.stats_available and self.stats_handler:
                try:
                    self._record_stats_safely(current_question, is_correct, user_answer, question_index)
                except Exception as e:
    ...
    def get_stats_data(self):
        ...
        if not (self.stats_available and self.stats_handler):
            return {}
    ...
    def _record_stats_safely(self, question_data, is_correct, user_answer, question_index):
        """StatsHandler 안전 호출 어댑터"""
        if not (self.stats_available and self.stats_handler):
            return
        
        if not hasattr(self.stats_handler, 'record_answer'):
            return
        
        # Week1 StatsHandler의 실제 시그니처 탐지 및 호출
        try:
            # 방법 1: question_data 파라미터 방식 시도
            self.stats_handler.record_answer(
                question_data=question_data,
                is_correct=is_correct,
                user_answer=user_answer
            )
            print("📊 StatsHandler 기록 성공 (question_data)")
            return
        except TypeError:
            pass
        ...
        try:
            ...
            self.stats_handler.record_answer(
                is_correct=is_correct,
                question_id=question_data.get('qcode', f'Q{question_index}'),
                category=question_data.get('layer1', '일반'),
                user_answer=user_answer
            )
            print("📊 StatsHandler 기록 성공 (개별 파라미터)")
            return
        except TypeError:
            pass
        ...
        try:
            # 방법 3: 최소 파라미터 방식 시도
            self.stats_handler.record_answer(is_correct, user_answer)
            print("📊 StatsHandler 기록 성공 (최소 파라미터)")
            return
        except TypeError:
            pass
        
        # 모든 방법 실패시 로그만 출력
        print("⚠️ StatsHandler.record_answer 시그니처 불일치 - 통계 기록 생략")