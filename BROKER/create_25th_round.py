...
# -*- coding: utf-8 -*-
"""
BROKER - Create 25th Round Data
기존 마스터 파일에 25회 데이터 추가
    ...
    if new_data_rows:
        ...
        try:
            ...
            for layer1 in ['관계법령', '손보1부', '손보2부']:
                layer_count = len(verification_df[(verification_df['EROUND'] == 25) & (verification_df['LAYER1'] == layer1)])
                print(f"  25회 {layer1}: {layer_count}개")
            
            # 전체 과목별 문제 수 확인
            print(f"\n=== 전체 현황 ===")
            for round_num in [20, 21, 22, 23, 24, 25]:
...
def main():
    """메인 실행 함수"""
    success = create_25th_round_data()
