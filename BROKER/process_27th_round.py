...
# -*- coding: utf-8 -*-
"""
BROKER - Process 27th Round
27회 PDF 파일을 처리하여 기존 행의 QUESTION 필드만 업데이트
...
def find_27th_pdf_files():
    """27회 PDF 파일 찾기"""
    pdf_files = []
    for file in os.listdir('.'):
        if file.endswith('.pdf') and '27회' in file:
...
def process_27th_round():
    ...
    # 1. 27회 PDF 파일 찾기
    pdf_files = find_27th_pdf_files()
    if not pdf_files:
        print("❌ 27회 PDF 파일을 찾을 수 없습니다.")
        return False
    
    print(f"발견된 27회 PDF 파일: {pdf_files}")
    ...
    for pdf_file in pdf_files:
        ...
        # 기존 27회 해당 과목 행 찾기
        target_mask = (df['EROUND'] == 27) & (df['LAYER1'] == layer1)
        target_rows = df[target_mask]
        
        if len(target_rows) == 0:
            print(f"❌ 27회 {layer1} 행을 찾을 수 없습니다.")
            continue
        
        print(f"기존 27회 {layer1} 행 수: {len(target_rows)}개")
    ...
    try:
        ...
        verification_mask = (verification_df['EROUND'] == 27)
        non_empty_questions = verification_df[verification_mask]['QUESTION'].notna().sum()
        total_27th_questions = len(verification_df[verification_mask])
        print(f"저장 후 27회 총 문제 수: {total_27th_questions}개")
        print(f"저장 후 27회 내용 있는 문제 수: {non_empty_questions}개")
        
        # 과목별 문제 수 확인
        for layer1 in ['관계법령', '손보1부', '손보2부']:
            layer_mask = (verification_df['EROUND'] == 27) & (verification_df['LAYER1'] == layer1)
            layer_count = len(verification_df[layer_mask])
            layer_filled = verification_df[layer_mask]['QUESTION'].notna().sum()
            print(f"  27회 {layer1}: {layer_filled}/{layer_count}개")
