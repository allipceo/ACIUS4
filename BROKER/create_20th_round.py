...
# -*- coding: utf-8 -*-
"""
BROKER - Create 20th Round Data
새로운 마스터 파일에 20회 데이터 생성
...
def create_20th_round_data():
    """20회 데이터 생성 메인 함수"""
    excel_file = "GEP_MASTER_V1.0.xlsx"  # .xlsx 확장자로 변경
    ...
    if common_files:
        ...
        if combined_text:
            ...
            if len(questions) > 0:
                ...
                print(f"✅ 20회 공통 처리 완료: {len(questions)}개 문제 생성")
            else:
    ...
    if data_rows:
        ...
        try:
            ...
            for layer1 in ['관계법령', '손보1부', '손보2부']:
                layer_count = len(verification_df[verification_df['LAYER1'] == layer1])
                print(f"  20회 {layer1}: {layer_count}개")
...
def main():
    """메인 실행 함수"""
    success = create_20th_round_data()
