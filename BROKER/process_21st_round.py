...
# -*- coding: utf-8 -*-
"""
BROKER - Process 21st Round
21회 PDF 파일을 처리하여 실제 문제 수에 맞게 행을 생성하고 문제 내용을 입력
...
def process_21st_round():
    ...
    for pdf_file in pdf_files:
        ...
        # 기존 21회 해당 과목 행 삭제 (있다면)
        existing_mask = (df['EROUND'] == 21) & (df['LAYER1'] == layer1)
        if existing_mask.any():
            df = df[~existing_mask]
            print(f"기존 21회 {layer1} 행 삭제 완료")
    ...
    try:
        ...
        verification_mask = (verification_df['EROUND'] == 21)
        total_21st_questions = len(verification_df[verification_mask])
        print(f"저장 후 21회 총 문제 수: {total_21st_questions}개")
        
        # 과목별 문제 수 확인
        for layer1 in ['관계법령', '손보1부', '손보2부']:
            layer_mask = (verification_df['EROUND'] == 21) & (verification_df['LAYER1'] == layer1)
