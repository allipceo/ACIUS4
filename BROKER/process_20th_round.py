...
# -*- coding: utf-8 -*-
"""
BROKER - Process 20th Round
20회 PDF 파일을 처리하여 기존 행의 QUESTION 필드만 업데이트
(공통은 두 개 파일을 이어서 처리)
...
def find_20th_pdf_files():
    """20회 PDF 파일 찾기"""
    pdf_files = []
    for file in os.listdir('.'):
        if file.endswith('.pdf') and '20회' in file:
...
def process_20th_round():
    ...
    # 1. 20회 PDF 파일 찾기
    pdf_files = find_20th_pdf_files()
    if not pdf_files:
        print("❌ 20회 PDF 파일을 찾을 수 없습니다.")
        return False
    
    print(f"발견된 20회 PDF 파일: {pdf_files}")
    ...
    # 공통 파일들을 먼저 찾아서 이어서 처리
    common_files = [f for f in pdf_files if '공통' in f]
    other_files = [f for f in pdf_files if '공통' not in f]
    
    # 공통 파일들 처리 (두 파일을 이어서)
    if common_files:
        print(f"\n=== 20회 공통 파일들 처리 시작 ===")
        print(f"공통 파일들: {common_files}")
        
        # 모든 공통 파일의 텍스트를 이어서 처리
        combined_text = ""
        for common_file in common_files:
            text = extract_text_from_pdf(common_file)
            if text:
                combined_text += text + "\n"
        
        if combined_text:
            # 문제 파싱
            questions = parse_questions_from_text(combined_text)
            print(f"추출된 문제 수: {len(questions)}개")
            
            if len(questions) > 0:
                # 기존 20회 관계법령 행 찾기
                target_mask = (df['EROUND'] == 20) & (df['LAYER1'] == '관계법령')
                target_rows = df[target_mask]
                
                if len(target_rows) > 0:
                    print(f"기존 20회 관계법령 행 수: {len(target_rows)}개")
                    ...
                    print(f"✅ 20회 공통 처리 완료: {updated_count}개 문제 업데이트")
                    total_processed += updated_count
                else:
                    print(f"❌ 20회 관계법령 행을 찾을 수 없습니다.")
            else:
                print(f"❌ 공통 파일들에서 문제를 추출할 수 없습니다.")
        else:
            print(f"❌ 공통 파일들 텍스트 추출 실패")
    
    # 나머지 파일들 처리 (손보1부, 손보2부)
    for pdf_file in other_files:
        ...
        # 기존 20회 해당 과목 행 찾기
        target_mask = (df['EROUND'] == 20) & (df['LAYER1'] == layer1)
        target_rows = df[target_mask]
        
        if len(target_rows) == 0:
            print(f"❌ 20회 {layer1} 행을 찾을 수 없습니다.")
            continue
        
        print(f"기존 20회 {layer1} 행 수: {len(target_rows)}개")
    ...
    try:
        ...
        verification_mask = (verification_df['EROUND'] == 20)
        non_empty_questions = verification_df[verification_mask]['QUESTION'].notna().sum()
        total_20th_questions = len(verification_df[verification_mask])
        print(f"저장 후 20회 총 문제 수: {total_20th_questions}개")
        print(f"저장 후 20회 내용 있는 문제 수: {non_empty_questions}개")
        
        # 과목별 문제 수 확인
        for layer1 in ['관계법령', '손보1부', '손보2부']:
            layer_mask = (verification_df['EROUND'] == 20) & (verification_df['LAYER1'] == layer1)
            layer_count = len(verification_df[layer_mask])
            layer_filled = verification_df[layer_mask]['QUESTION'].notna().sum()
            print(f"  20회 {layer1}: {layer_filled}/{layer_count}개")
