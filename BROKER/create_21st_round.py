...
# -*- coding: utf-8 -*-
"""
BROKER - Create 21st Round Data
기존 마스터 파일에 21회 데이터 추가
...
def find_21st_pdf_files():
    """21회 PDF 파일 찾기"""
    pdf_files = []
    for file in os.listdir('.'):
        if file.endswith('.pdf') and '21회' in file:
...
def create_21st_round_data():
    ...
    # 2. 21회 PDF 파일 찾기
    pdf_files = find_21st_pdf_files()
    if not pdf_files:
        print("❌ 21회 PDF 파일을 찾을 수 없습니다.")
        return False
    
    print(f"발견된 21회 PDF 파일: {pdf_files}")
    ...
    if common_files:
        ...
        if combined_text:
            ...
            if len(questions) > 0:
                ...
                print(f"✅ 21회 공통 처리 완료: {len(questions)}개 문제 생성")
            else:
    ...
    if new_data_rows:
        ...
        try:
            ...
            for layer1 in ['관계법령', '손보1부', '손보2부']:
                layer_count = len(verification_df[(verification_df['EROUND'] == 21) & (verification_df['LAYER1'] == layer1)])
                print(f"  21회 {layer1}: {layer_count}개")
...
def main():
    """메인 실행 함수"""
    success = create_21st_round_data()
