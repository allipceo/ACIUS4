# app_v2.7.py - AICU S4 기본학습 문제 로딩 실패 해결 버전
...
def create_app():
    """AICU S4 v2.7 - 기본학습 문제 로딩 실패 해결 버전"""
    app = Flask(__name__)
    ...
    # 🔧 v2.7 추가: 통계 API 권한 검증 개선
    @app.route('/api/user/statistics/<user_id>', methods=['GET'])
    def get_user_statistics_fixed(user_id):
        """사용자별 통계 조회 - 게스트 모드 권한 검증 개선"""
        print(f"=== 사용자 통계 조회 (v2.7): {user_id} ===")
        
        try:
            current_user_id = session.get('current_user_id')
            
            # 🔧 v2.7 수정: 게스트 모드 권한 검증 개선
            if current_user_id != user_id:
                # 게스트 모드인 경우 자신의 통계만 조회 가능
                if current_user_id.startswith('guest_') and user_id.startswith('guest_'):
                    if current_user_id != user_id:
                        print(f"⚠️ 게스트 권한 불일치: 세션={current_user_id}, 요청={user_id}")
                        return jsonify({
                            'success': False,
                            'error': '게스트 권한이 없습니다.'
                        }), 403
                # 실제 사용자인 경우 자신의 통계만 조회 가능
                elif not current_user_id.startswith('guest_') and not user_id.startswith('guest_'):
                    if current_user_id != user_id:
                        print(f"⚠️ 사용자 권한 불일치: 세션={current_user_id}, 요청={user_id}")
                        return jsonify({
                            'success': False,
                            'error': '권한이 없습니다.'
                        }), 403
                else:
                    print(f"⚠️ 게스트-사용자 권한 불일치: 세션={current_user_id}, 요청={user_id}")
                    return jsonify({
                        'success': False,
                        'error': '권한이 없습니다.'
                    }), 403
    ...
    def get_question_fixed(session_id, index):
        ...
        try:
            ...
            if not session_id or '_' not in session_id:
                print(f"❌ 잘못된 세션 ID 형식: {session_id}")
                return jsonify({
                    'success': False,
            ...
            # 🔧 v2.7 수정: 게스트 모드 세션 검증
            if current_user_id.startswith('guest_'):
                # 게스트 모드에서는 간단한 세션 검증
                if not session_id.startswith(current_user_id):
                    print(f"⚠️ 게스트 세션 불일치: 세션={current_user_id}, 요청={session_id}")
                    return jsonify({
                        'success': False,
                        'error': '게스트 세션이 일치하지 않습니다',
                        'code': 'GUEST_SESSION_MISMATCH'
                    }), 401
            else:
                # 실제 사용자 모드에서는 정확한 세션 검증
                if not session_id.startswith(current_user_id):
                    print(f"⚠️ 사용자 세션 불일치: 세션={current_user_id}, 요청={session_id}")
                    return jsonify({
                        'success': False,
                        'error': '사용자 세션이 일치하지 않습니다',
                        'code': 'USER_SESSION_MISMATCH'
                    }), 401
            
            # 🔧 v2.7 수정: 문제 데이터 로드 (JSON 파일에서)
            try:
                import json
...
if __name__ == '__main__':
    app = create_app()
    print("=" * 60)
    print("🚀 AICU S4 v2.7 FINAL (기본학습 문제 로딩 실패 해결)")
    print("📍 URL: http://localhost:5000")
    print("📋 v2.7 개선 사항:")
    print("   ✅ 게스트 모드 세션 ID 형식 통일")
    print("   ✅ 통계 API 권한 검증 개선 (403 FORBIDDEN 해결)")
    print("   ✅ 문제 로딩 API 세션 검증 강화 (401 UNAUTHORIZED 해결)")
    print("   ✅ 디버그 API 추가 (/api/debug/session)")
    print("   ✅ JSON 파일 직접 로딩으로 문제 데이터 접근성 향상")
    print("=" * 60)
